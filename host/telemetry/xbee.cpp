...
xbee::xbee(std::string serial_device_filename,
  ...
  serial_port_.set_option(boost::asio::serial_port_base::baud_rate(
      baud_rate));
  serial_port_.set_option(boost::asio::serial_port_base::character_size(
      8));
...
void xbee::write(char c) {
  boost::asio::write(serial_port_, boost::asio::buffer(&c, 1));
}
...
std::string xbee::get_line(const char end) {
  boost::asio::read_until(serial_port_, streambuf_, "\r\n");
